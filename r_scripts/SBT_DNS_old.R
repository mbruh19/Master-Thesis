...
final_table <- final_table %>%
  mutate(`16` = coalesce(`16`, `16-16`),
         `32` = coalesce(`32`, `32-32`),
         `64` = coalesce(`64`, `64-64`),
         `128` = coalesce(`128`, `128-128`),
         `256` = coalesce(`256`, `256-256`)) %>%
  select(ObjectiveFunction, NetworkStructure, `16`, `32`, `64`, `128`, `256`)
...
caption <- "The mean test accuracies on MNIST for different network structures. The results are obtained by training a BNN
            on a single batch with 1000 samples using the ILS algorithm with a time limit of 120 seconds.
