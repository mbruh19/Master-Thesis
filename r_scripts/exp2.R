...
setwd("C:/Users/Mads/OneDrive/SDU/Thesis/bnn/log/exp2")
files <- list.files()
...
ggsave(filename = "C:/Users/Mads/OneDrive/SDU/Thesis/bnn/tex/Thesis/Figures/SBT_TEFL.png", plot = p, dpi = 300, bg = "white")

summary_stats_local_optimas <- results %>%
  group_by(ObjectiveFunction, TimeLimit) %>%
  summarise(
...
# Plot with standard deviation as a shaded area
p2 <- ggplot(summary_stats_local_optimas, aes(x = TimeLimit, y = Mean, group = ObjectiveFunction, color = ObjectiveFunction)) +
  geom_ribbon(aes(ymin = Mean - SD, ymax = Mean + SD, fill = ObjectiveFunction), alpha = 0.2) +
  geom_line() +  # Connect points with lines
  geom_point() +  # Show individual data points
  labs(title = "Number of local optimas for different objective functions for different time limits",
       x = "Time limit",
       y = "Local optimas") +
  scale_x_continuous(breaks = seq(10, 100, 10)) +  # Adjust x-axis breaks if necessary
  scale_y_continuous(breaks = seq(0, 500, 50)) +
  coord_cartesian(xlim = c(10, 101), ylim = c(0, 500))+
  theme_minimal() +
  theme(legend.title = element_blank(), 
        legend.position = "bottom",
        plot.title = element_text(hjust = 0.5),
...
ggsave(filename = "C:/Users/Mads/OneDrive/SDU/Thesis/bnn/tex/Thesis/Figures/exp2_local_optimas.png", plot = p2, dpi = 300, bg = "white")
