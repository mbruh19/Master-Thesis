...
set lfstart=../log/BEMI_OBJ_FMNIST/

for /L %%s in (0, 1, 4) do (
    for /L %%b in (0, 1, 1) do (
        for /L %%o in (2, 1, 2) do (
            set lf=!lfstart!!seeds[%%s]!_!batchsizes[%%b]!_!smtls[%%r]!_!objectives[%%o]!
            python -m main -ct !ct! -nt !nt! -ns !ns! -ds !ds! -vp !vp! -bs !batchsizes[%%b]! -ts !ts! -st !st! -ot !objectives[%%o]! -so !so! -al !al! -se !seeds[%%s]! -tl !tl! -ps !ps! -l !l! -lf !lf! -smtl !smtl! -vs !vs!
            echo !lf!
        )
    )     
)
